...
For each of the following grids, the rows may be rearranged to form a magic square.
...
If you take on this challenge, include the result you get for as many of the challenge input grids[RES ignored duplicate link] as you can, along with your code.
...
class MagicSquare(object):
	...
	def main(self):
		...
		self._sum = int(self._size*(self._size*self._size + 1)/2)
		validSquare = self.VerifyInput(self._rows)
		if (validSquare):
			print("Size: ", self._size)
			print("Sum: ", self._sum)
			if (self.SumDiagonals(self._rows)):
				self.OutputCurrentPermy()
	...
	def OutputCurrentPermy(self):
		print("Success!")
	
	def SwapRows(self, a, b):
		self._rows[a], self._rows[b] = self._rows[b], self._rows[a]
			
	def SumDiagonals(self, rows):
		forward = 0
		back = 0
		for i, row in enumerate(rows):
			numbers = row.rsplit(' ')
			forward += int(numbers[i])
			back += int(numbers[self._size-i-1])
		if (forward == self._sum and back == self._sum):
			return True
		else:
			return False
		
	def VerifyInput(self, rows):
		#Currently only verifies rows, not columns
		row = []
		for line in rows:
			row = line.rsplit(' ')
